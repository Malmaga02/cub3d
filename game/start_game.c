...
{
	draw_minimap(pAll);
	mlx_put_image_to_window(pAll->window.mlx, pAll->window.mlx_win, pAll->window.frame, 0, 0);
	return (0);
}
...
void	start_game(t_all *pAll)
{
	...
	mlx_hook(pAll->window.mlx_win, KeyRelease, KeyReleaseMask, on_key_release, pAll);
